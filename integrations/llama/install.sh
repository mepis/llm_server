...
if [ "$choice" == "y" ]; then
  ...
  cmake -B build -DGGML_CCACHE=on -DGGML_NATIVE=on -DGGML_LTO=on -DGGML_CUDA=on -DGGML_CUDA_FA=on -DGGML_CUDA_GRAPHS=on  -DGGML_CUDA_FA_ALL_QUANTS=on -DGGML_CUDA_NCCL=ON -DGGML_CUDA_PEER_MAX_BATCH_SIZE=256
  
  #  -DGGML_CUDA_FA_ALL_QUANTS=on -DGGML_CUDA_FORCE_MMQ=on -DCMAKE_CUDA_ARCHITECTURES="86;120" -DGGML_CUDA_PEER_MAX_BATCH_SIZE=512 -DGGML_CCACHE=on -DGGML_LTO=on -DGGML_CUDA_FORCE_CUBLAS=on -DGGML_BLAS=ON -DGGML_BLAS_VENDOR=Intel10_64lp -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx  -DGGML_CUDA_FORCE_MMQ=on 
