...
if [ "$choice" == "y" ]; then
  ...
  cmake -B build -DGGML_CCACHE=on -DGGML_NATIVE=on -DGGML_LTO=on -DGGML_CUDA=on -DGGML_CUDA_FA=on -DGGML_CUDA_GRAPHS=on -DGGML_CUDA_NCCL=ON -DGGML_CUDA_PEER_MAX_BATCH_SIZE=512
