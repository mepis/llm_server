...
./llama-server -m $MODEL_DIR/$model --mmproj $MODEL_DIR/$mmproj --port $port --host $host -c $context -ngl 999 --cont-batching --temp $temp --top-p $topP --min-p $minP --top-k $topK --batch-size 16384 --ubatch-size 16384 --flash-attn 1 --reasoning 1 --split-mode tensor --spec-type draft-mtp --spec-draft-n-max 3 -e --cache-reuse 512 --chat-template-kwargs '{"enable_thinking":false}' --presence-penalty 0.0 --repeat-penalty 1.0 --spec-draft-p-min 0.75
