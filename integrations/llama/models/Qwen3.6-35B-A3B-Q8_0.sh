...
./llama-server -m $MODEL_DIR/$model --mmproj $MODEL_DIR/$mmproj --port $port --host $host -c $context -ngl 999 --cont-batching --temp $temp --top-p $topP --min-p $minP --top-k $topK --batch-size 1024 --ubatch-size 1024 --flash-attn on --reasoning on --split-mode tensor -e --chat-template-kwargs '{"preserve_thinking":true}' --presence-penalty 0.0 --threads 16 --prio 2 --cpu-range 0-8 --cpu-strict 1 
