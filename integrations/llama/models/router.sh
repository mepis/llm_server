...
./llama-server --models-dir $MODEL_DIR --models-autoload --models-max 1 --sleep-idle-seconds 60 --port $port --host $host -c $context -ngl 999 --cont-batching --parallel 2 --temp $temp --top-p $topP  --min-p $minP --top-k $topK --batch-size 512 --ubatch-size 256 --kv-unified --flash-attn on --reasoning on --cache-prompt --rope-scaling yarn --rope-scale 2.0 --mirostat 2 --split-mode $splitMode --tensor-split $tensorSplit --main-gpu $mainGpu --cpu-range 0-7 --cpu-strict-batch 1 --threads-batch 8 --threads $threads --cpu-strict 1 --prio 2 --poll 30
