...
./llama-server --models-dir $MODEL_DIR --models-autoload --models-max 2 --sleep-idle-seconds 300 --port $port --host $host -c $context -ngl 999  --temp $temp --top-p $topP --cont-batching --min-p $minP --top-k $topK --parallel 2  --batch-size 1024 --ubatch-size 256 --kv-unified --chat-template-kwargs '{"enable_thinking":true}' --flash-attn on --reasoning on --cache-prompt --rope-scaling yarn --rope-scale 2.0 --mirostat 2 --split-mode $splitMode --tensor-split $tensorSplit --main-gpu $mainGpu --cpu-range 0-7 --cpu-strict-batch 1 --threads-batch 8 --threads $threads --cpu-strict 1 --prio 2 --poll 30 --predict 2 --n-cpu-moe 0 --ctx-checkpoints 64
