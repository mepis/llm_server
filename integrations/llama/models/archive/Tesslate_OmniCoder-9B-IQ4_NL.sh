#!/bin/bash
model=Tesslate_OmniCoder-9B-IQ4_NL.gguf
...
./llama-server -m $MODEL_DIR/$model  --port $port --host $host -c $context -ngl 999 --main-gpu $mainGpu --temp $temp --top-p $topP --cont-batching --min-p $minP --top-k $topK --threads $threads --prio 2 --cpu-range 0-7 --cpu-strict 1 --swa-full --kv-unified --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 4096 --ubatch-size 1024 --chat-template-kwargs '{"enable_thinking":true}' --presence-penalty 1 --repeat-penalty 1