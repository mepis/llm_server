Develop an implentation plan to add RAG operations and per-user persistant memory support.

**Target Audience**: Coding agents running on a local instance of llama.cpp with limited resources (smaller context window)

# Feature Requirements

- Support for these file types: pdf, docx, xlsx, csv, txt, md
- Document groups with role-based access controls
- Chat responses **must** include citations and links to sources
- Support multi-layer memory systems for individual users

# Technical Requirements

- Self-host Mongo 8.2 instance
- Llama.cpp support (OpenAI-compatible API)
...
2. Research
   ...
   - Persistant memory layers for LLMs in 2026
   - Current state of frontend and backend
3. Develop an implementation plan. Consider the info discovered in step 1.
