...
# Multi GPU (3x - 16gb, 12gb, 12gb) w/ higgingface import
GGML_CUDA_ENABLE_UNIFIED_MEMORY=1 ~/llm_server/llama.cpp/build/bin/./llama-server -hf $model --port 3000 --host 100.115.205.84 -c 131072 -ngl 99 --split-mode layer --tensor-split 16,12,12 --main-gpu 0 -b 2048 -ub 1024 -ctk bf16 --threads $threads --temp 0.7 --repeat-penalty 1.0 --presence-penalty 1.5 --top-p 0.8 --min-p 0.0 --flash-attn 0 --webui --reasoning-budget 0 --parallel 4 --cont-batching --kv-unified
...
# local laptop
# llama-server -hf unsloth/Qwen3.5-9B-GGUF:UD-Q4_K_XL --port 3000 --host 100.110.63.5 -b 2048 -ub 1024 -ctk bf16 --threads 28 --temp 0.7 --repeat-penalty 1.0 --presence-penalty 1.5 --top-p 0.8 --min-p 0.0 --flash-attn 0 --webui --reasoning-budget 0 --parallel 2 --cont-batching --kv-unified