# Results Binary Documentation

## Overview

The `llama-results` binary verifies and checks model results. It can validate logits and compare model outputs against expected results.
...
| `--check` | Check results rather than generate | 0 |
| `--save-logits` | Save logits to files | 0 |
| `--logits-output-dir` | Directory for logits output | data |
| `--tensor-filter` | Filter tensor names (regex) | - |
...
```bash
# Check model results
./llama-results -m model.gguf --check
```
