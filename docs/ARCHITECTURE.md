...
LLM Server employs a modular architecture composed of several interconnected components:

### 1. Core Components

- **API Server**: Go-based REST API server implementing OpenAI-compatible endpoints
- **llama.cpp Integration**: Core LLM inference engine providing model loading and text generation
- **Frontend Application**: Vue 3 + Vite web interface for user interaction
- **Database Layer**: MySQL-backed authentication and session management
- **Build System**: CMake-based compilation for high-performance inference

### 2. Data Flow

1. **User Authentication**: User credentials verified against MySQL database
2. **Token Generation**: Successful authentication returns session token
3. **API Request Processing**: Authenticated requests proceed to appropriate endpoint
4. **Configuration Loading**: Request parameters merged with defaults
5. **Model Invocation**: llama.cpp library processes request using configured parameters
6. **Response Generation**: Text generation with specified parameters
7. **Response Formatting**: API response formatted according to OpenAI specification
8. **Response Delivery**: Generated response sent back to client

### 3. Component Interactions

```
+----------------+     +----------------+     +-----------------+
|   Frontend     |<--->|   API Server   |<--->|   llama.cpp     |
+----------------+     +----------------+     +-----------------+
         ^                         ^                       ^
         |                         |                       |
         +--- HTTP Requests ------+ |                       |
...
1. **CMake Configuration**: Generate build configuration with desired options
2. **Compilation**: Build llama.cpp core with optimizations
3. **API Compilation**: Compile Go API server with C dependencies
4. **Frontend Build**: Build Vue 3 application for production
5. **Artifact Packaging**: Collect all necessary components for deployment

### 5. Runtime Process

1. **Service Initialization**: Load configuration and database connections
2. **HTTP Server Startup**: Listen on configured port
3. **Request Handling**: Process incoming API requests
4. **Authentication Verification**: Validate Bearer tokens
5. **Model Parameter Processing**: Merge user-specified parameters with defaults
6. **Text Generation**: Invoke llama.cpp inference engine
7. **Response Construction**: Format output according to API specification
8. **Response Delivery**: Send HTTP response to client

### 6. Module Dependencies

- **API Server**: Depends on llama.cpp library, MySQL client, Go standard library
- **llama.cpp Integration**: Depends on CMake build system, system libraries
- **Frontend Application**: Depends on Node.js, Vue 3, Vite
- **Database Layer**: Depends on MySQL server, Go SQL driver
- **Build System**: Depends on CMake, compiler toolchain
