# Export LoRA Binary Documentation

## Overview

The `llama-export-lora` binary exports LoRA (Low-Rank Adaptation) adapters from llama models. This is useful for creating custom model adaptations.
...
```bash
# Export LoRA from a model
./llama-export-lora -m base_model.gguf -f training_data.txt -o lora.gguf
```
