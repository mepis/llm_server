jon@betty:~/llm_server/scripts$ ~/llm_server/llama.cpp/build/bin/./llama-server --help
...
(env: LLAMA_ARG_DIO)
--numa TYPE attempt optimizations that help on some NUMA systems - distribute: spread execution evenly over all nodes - isolate: only spawn threads on CPUs on the node that execution
started on - numactl: use the CPU map provided by numactl
...
-sm, --split-mode {none,layer,row} how to split the model across multiple GPUs, one of: - none: use one GPU only - layer (default): split layers and KV across GPUs - row: split rows across GPUs
...
ignored. Values: - 0: generic output - 1: error - 2: warning - 3: info - 4: debug
(default: 3)
...
max number of context checkpoints to create per slot (default: 8)[(more info)](https://github.com/ggml-org/llama.cpp/pull/15293)
