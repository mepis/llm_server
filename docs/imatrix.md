# IMatrix Binary Documentation

## Overview

The `llama-imatrix` binary computes importance matrices (imatrix) for quantization. The imatrix is used to determine which weights are most important to preserve during quantization, enabling better quality at lower bitrates.
...
| `--in-file` | Input file (comma-separated) | - |
| `--chunks` | Max chunks to process | -1 |
| `--chunk` | Start from chunk N | 0 |
...
| `--output-format` | Output format (gguf/dat) | gguf |
| `--ofreq` | Output frequency | 10 |
| `--save-frequency` | Save every N iterations | 0 |

### Processing Options

| Flag | Description | Default |
|------|-------------|---------|
| `--process-output` | Process output tensor | 0 |
| `--ppl` | Compute perplexity | 1 |
| `--no-ppl` | Disable perplexity | 0 |
| `--show-statistics` | Show statistics and exit | 0 |
...
```bash
# Compute imatrix from a text file
./llama-imatrix -m model.gguf -f corpus.txt -o imatrix.dat

# Output in GGUF format
./llama-imatrix -m model.gguf -f corpus.txt -o imatrix.gguf --output-format gguf

# Show statistics
./llama-imatrix -m model.gguf -f corpus.txt --show-statistics
```
