# GGUF Split Binary Documentation

## Overview

The `llama-gguf-split` binary splits GGUF model files into multiple parts. This is useful for distributing large models or preparing them for specific use cases.
...
| `--split-threshold` | Split threshold in bytes | 2048 |
| `--part-size` | Part size in bytes | - |
...
```bash
# Split model into 2GB parts
./llama-gguf-split -m model.gguf -o model-part-00001-of-00005.gguf --part-size 2147483648

# Split by tensor count
./llama-gguf-split -m model.gguf -o model-part-00001-of-00005.gguf --split-threshold 2048
```
