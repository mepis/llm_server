# Batched Bench Binary Documentation

## Overview

The `llama-batched-bench` binary benchmarks batched inference performance. It measures throughput and latency for batched requests.
...
| `-pps` | Shared prompt across parallel | 0 |
| `-tgs` | Separate text generation | 0 |
...
./llama-batched-bench -m model.gguf -npp 512 -ntg 512 -npl 4

# JSON output
./llama-batched-bench -m model.gguf -npp 512 -ntg 512 --output-format jsonl
```
