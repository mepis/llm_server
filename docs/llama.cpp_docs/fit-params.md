# Fit Params Binary Documentation

## Overview

The `llama-fit-params` binary fits model parameters to available device memory. It helps optimize memory usage when running models on limited hardware.
...
| Flag | Description | Default |
|------|-------------|---------|
| `-lr, --learning-rate` | Learning rate (alpha) | 1e-5 |
| `-lr-min, --learning-rate-min` | Minimum learning rate | -1 |
| `-decay-epochs, --learning-rate-decay-epochs` | Decay epochs | -1 |
| `-wd, --weight-decay` | Weight decay | 0 |
| `-val-split, --val-split` | Validation split fraction | 0.05 |
| `-epochs, --epochs` | Number of epochs | 2 |
| `-opt, --optimizer` | Optimizer (sgd/adamw) | adamw |
...
```bash
# Fit parameters for training
./llama-fit-params -m model.gguf -lr 1e-5 -epochs 10
```
