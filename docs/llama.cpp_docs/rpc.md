# RPC Binary Documentation

## Overview

The `llama-rpc` binary provides RPC (Remote Procedure Call) functionality for llama models. It enables distributed inference across multiple machines.
...
| Flag | Description | Default |
|------|-------------|---------|
| `--rpc` | Comma-separated RPC servers (host:port) | - |
...
./llama-rpc -m model.gguf --rpc server1:5000,server2:5000
```
