...
- Scripts to clone the llama.cpp repository, analyze the local system hardware, and build an optimized copy of llama.cpp based on the local hardware
- Scripts to launch Llama.cpp with optimized settings for the local system
  -- If the local system has an nvidia gpu, the scripts should use unified memory and avoid using the CPU
- Scripts that should install a system service to run llama.cpp on start based on saved configs (use root as the user)
- Scripts that should install a system service to run the frontend (use root as the user)
- A script that updates this repo (pulls the latests commits), and builds the frontend
- A frontend using Vue.js to run these scripts.
- The frontend should use a mint and white theme
- the frontend should include functions to download models from huggingface
- the front end needs to include a sidebar for viewing and running pages and commands
- the frontend should display system metrics such as memory usage, cpu speed, gpu usage, etc.
- the frontend should include tools to download and manage models from huggingface
- the frontend should include searh features for models from huggingface
- include documentation in the frontend
- the frontend should be accesible from other computers over the network

## Application Requirements

- save configs and settings to a sqllite db
